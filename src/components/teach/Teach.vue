<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>教学列表</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column label="标题" prop="teachTitle"> </el-table-column>
        <el-table-column label="内容" prop="teachContent"> </el-table-column>
      ...
      >
        ...
        >
          <el-form-item label="标题" prop="username">
            <el-input v-model="addForm.teachTitle"></el-input>
          </el-form-item>
          <el-form-item label="内容" prop="nickname">
            <el-input type="textarea" v-model="addForm.teachContent"></el-input>
          </el-form-item>
        </el-form>
        <!-- 底部区域 -->
      ...
      >
        ...
        >
          <el-form-item label="标题">
            <el-input v-model="showForm.teachTitle" disabled></el-input>
          </el-form-item>
          <el-form-item label="内容">
            <el-input
              type="textarea"
              v-model="showForm.teachContent"
              disabled
            ></el-input>
          </el-form-item>
        </el-form>
      ...
      >
        ...
        >
          <el-form-item label="标题">
            <el-input v-model="editForm.teachTitle"></el-input>
          </el-form-item>
          <el-form-item label="内容">
            <el-input
              type="textarea"
              v-model="editForm.teachContent"
            ></el-input>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    async getTeachList() {
      ...
      const { data } = await this.$http.get("teach/list", {
        params: {
          pageNum: this.queryInfo.pagenum,
          pageRow: this.queryInfo.pagesize,
        },
      });
      console.log("教学信息目录", data);
      if (data.code == "200") {
    ...
    async seekUserList() {
      ...
      const { data: data } = await this.$http.get("teach/listByTeachTitle", {
        params: {
          teachTitle: this.seekvalue,
          pageNum: "1",
          pageRow: "10",
        },
      });
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        // 发起修改用户信息请求
        let req = {
          teachTitle: this.editForm.teachTitle,
          teachContent: this.editForm.teachContent,
          teachId: this.editForm.teachId,
          deleteStatus: "1",
        };
        const { data: data } = await this.$http.post("teach/updateTeach", req);
        // console.log(data);
    ...
    async deleteUser() {
      ...
      const { data: data } = await this.$http.post("teach/batchDeleteTeach", {
        teachId: this.deletecheckedUsers,
        deleteStatus: "2",
      });
      console.log(data);
      this.getTeachList();
    ...
    async setRole(item) {
      console.log(item);
      this.$confirm("是否删除该教学, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(async () => {
          const { data: data } = await this.$http.post("teach/updateTeach", {
            teachTitle: item.teachTitle,
            teachContent: item.teachContent,
