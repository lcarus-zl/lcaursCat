    ...
    <el-card>
      ...
      >
        ...
        >
          ...
          <el-form-item label="昵称" prop="nickname">
            <el-input v-model="showForm.nickname" disabled></el-input>
          </el-form-item>
  ...
  methods: {
    ...
    addUser() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        if (!valid) return this.$message.error("用户信息错误，添加用户失败");
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async (valid) => {
        if (!valid) return this.$message.error("用户信息错误，修改用户失败");
        // 发起修改用户信息请求
        let req = {
        ...
        const { data: data } = await this.$http.post("user/updateUser", req);
        console.log(data);
    ...
    async deleteUser() {
      ...
      const { data: data } = await this.$http.post("user/batchDeleteUser", {
        userId: this.deletecheckedUsers,
        deleteStatus: "2",
      });
      console.log("批量删除用户", data);
      this.getUserList();
    ...
    async removeUserById(id) {
      const confirmResult = await this.$confirm("是否删除该用户?", "删除用户", {
    ...
    async setRole(item) {
      ...
      this.$confirm("是否删除该用户, 是否继续?", "提示", {
        ...
        .then(async () => {
          const { data: data } = await this.$http.post("user/updateUser", {
            userId: item.userId,
